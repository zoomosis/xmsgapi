...
static unsigned _SquishOpenBaseFiles(HAREA ha, byte * szName, int mode)
{
    char szFile[PATHLEN];

    strcpy(szFile, (char *) szName);
    strcat(szFile, dot_sqd);
...
static unsigned _SquishUnlinkBaseFiles(byte * szName)
{
    char szFile[PATHLEN];
    unsigned rc = TRUE;
...
static void _SquishCloseBaseFiles(HAREA ha)
{
    close(Sqd->sfd);
    close(Sqd->ifd);
...
static unsigned _SquishValidateBaseHeader(SQBASE * psqb)
{
    if (psqb->num_msg > psqb->high_msg || psqb->num_msg > psqb->uid + 1 ||
      psqb->high_msg > psqb->uid + 1 || psqb->num_msg > 1000000L ||
      psqb->num_msg != psqb->high_msg || psqb->len < SQBASE_SIZE ||
      psqb->len >= 1024 || psqb->begin_frame > psqb->end_frame ||
      psqb->last_frame > psqb->end_frame || psqb->free_frame > psqb->end_frame ||
      psqb->last_free_frame > psqb->end_frame || psqb->end_frame == 0)
    {
        msgapierr = MERR_BADF;
        return FALSE;
    }
...
static unsigned _SquishSetBaseDefaults(HAREA ha)
{
...
static unsigned _SquishFillBaseHeader(SQBASE * psqb)
{
    memset(psqb, 0, sizeof *psqb);
...
static unsigned _SquishCreateNewBase(HAREA ha, byte * szName)
{
...
static unsigned _SquishOpenExistingBase(HAREA ha, byte * szName)
{
    SQBASE sqb;  /* Header from Squish base */
...
static HAREA NewHarea(word wType)
{
    ...
    ha->type = (word) (wType & ~MSGTYPE_ECHO);
...
HAREA MSGAPI SquishOpenArea(byte * szName, word wMode, word wType)
{
    ...
    *ha->api = sq_funcs;
    ...
    if (msgapierr == MERR_NOENT && (wMode == MSGAREA_CREATE ||
...
static unsigned _SquishCloseAreaMsgs(HAREA ha)
{
    HMSG hm, hmNext;
...
static unsigned _SquishRemoveAreaList(HAREA haThis)
{
    HAREA ha, haNext;
...
sword apiSquishCloseArea(HAREA ha)
{
