...
static unsigned _SquishProbeFreeChain(HAREA ha, dword dwLen, FOFS * pfo,
  SQHDR * psqh, dword * pdwFrameLen)
{
    ...
    *pdwFrameLen = 0L;
    ...
    for (foThis = Sqd->foFree; foThis != NULL_FRAME;
      foLast = foThis, foThis = psqh->next_frame)
    {
        ...
        /*
         *  Verify that this is a valid frame and that we aren't going
         *  around in circles.
         */
...
static unsigned _SquishRemoveFreeChain(HAREA ha, FOFS fo, SQHDR * psqh)
{
    assert(Sqd->fHaveExclusive);
...
/*
 *  Allocate a frame of length dwLen at end-of-file, and store result in
 *  pfo.  This function assumes that we have exclusive access to the Squish
 *  base.
 */

static unsigned _SquishGetFrameEOF(HAREA ha, FOFS * pfo, dword dwLen)
{
    char nul = 0;
    long ofs;
    ...
    if (lseek(Sqd->sfd, ofs, SEEK_SET) != ofs || write(Sqd->sfd, &nul, 1) != 1)
    {
        msgapierr = MERR_NODS;
        return FALSE;
    }
...
static unsigned _SquishGetNewFrame(HMSG hmsg, dword dwLen, FOFS * pfoNew,
  dword * pdwFrameLen)
{
    SQHDR sqh;
    FOFS fo;
 ...
 *  is blank, just append the message to the end of the chain.  However, if
 *  foRead is NOT blank, we should set the new frame so that it will appear
 *  between hmsg->sqhRead.prev and hmsg->sqhRead.next, as we are
...
static unsigned _SquishLinkMessageFrame(HMSG hmsg, dword dwTotal,
  dword dwCtrlLen, dword dwFrameLen)
{
    ...
    if (hmsg->sqhWrite.prev_frame == NULL_FRAME)
    {
         ...
         *  have either replaced the first message, or we are simply
         *  creating the first message in a Squish base.
         */
    ...
    if (hmsg->sqhWrite.next_frame == NULL_FRAME)
    {
         ...
         *  either added a new message, or replaced the old end-of-file
         *  message.
         */
 ...
 *  allocate it.  This function assumes that we have exclusive access to
 *  the Squish base.
 */

static unsigned _SquishGetWriteFrame(HMSG hmsg, dword dwTxtTotal,
  dword dwCtrlLen)
{
    ...
    /*
     *  If we're writing over an existing message, verify that the total
     *  and control lengths are not more than what we have already.
     */
    ...
    if (hmsg->wMode == MOPEN_RW || hmsg->wMode == MOPEN_WRITE)
    {
        ...
        /*
         *  Copy the read-mode parameters, since we are now writing over
         *  this message at the specified offset.
         */
    ...
    {
        /*
         *  First, if we are replacing an existing message, release its
         *  frame to the free chain.
         */
        ...
        {
            /*
             *  That failed, so we can't write the message.  However, if we 
             *  were trying to replace an existing message, we have added
             *  that message to the free chain, but we have NOT linked the
             *  other messages to skip over it.  The only recourse is to do
             *  the linking and remove it from the index file (effectively
            ...
            if (hmsg->foRead)
            {
                ...
                _SquishSetFrameNext(hmsg->ha, hmsg->sqhRead.prev_frame,
                  hmsg->sqhRead.next_frame);

                _SquishSetFramePrev(hmsg->ha, hmsg->sqhRead.next_frame,
                  hmsg->sqhRead.prev_frame);

                /* Now remove the old one from the index */

                _SquishRemoveIndexEntry(HSqd->hix, hmsg->dwMsg, NULL,
                  &hmsg->sqhRead, TRUE);
            }
...
static int write_xmsg(int handle, XMSG * pxmsg)
{
    ...
    rawdate |= (word) (((word) pxmsg->date_written.date.da) & 31);
    rawdate |= (word) ((((word) pxmsg->date_written.date.mo) & 15) << 5);
    rawdate |= (word) ((((word) pxmsg->date_written.date.yr) & 127) << 9);

    rawtime |= (word) (((word) pxmsg->date_written.time.ss) & 31);
    rawtime |= (word) ((((word) pxmsg->date_written.time.mm) & 63) << 5);
    rawtime |= (word) ((((word) pxmsg->date_written.time.hh) & 31) << 11);
    ...
    rawdate |= (word) (((word) pxmsg->date_arrived.date.da) & 31);
    rawdate |= (word) ((((word) pxmsg->date_arrived.date.mo) & 15) << 5);
    rawdate |= (word) ((((word) pxmsg->date_arrived.date.yr) & 127) << 9);

    rawtime |= (word) (((word) pxmsg->date_arrived.time.ss) & 31);
    rawtime |= (word) ((((word) pxmsg->date_arrived.time.mm) & 63) << 5);
    rawtime |= (word) ((((word) pxmsg->date_arrived.time.hh) & 31) << 11);
    ...
    return (write(handle, (byte *) buf, XMSG_SIZE) == XMSG_SIZE);
}

/* Write the XMSG header to the Squish file */

static unsigned _SquishWriteXmsg(HMSG hmsg, PXMSG pxm, dword * pdwOfs)
{
    /* XMSG xmsg; */
...
#define xmsg (*pxm)
     ...
     *  use it to restore the index file, if necessary.  However, if 
     *  the umsgid is not known, make sure that the MSGUID bit is not
     *  set.
     */
...
static unsigned _SquishWriteCtrl(HMSG hmsg, byte * szCtrl, dword dwCtrlLen,
  dword * pdwOfs)
{
    long ofs;
    ...
    /*
     *  Make sure that the control information is not longer than what we 
     *  wrote the first time, if we're updating a message.
     */
...
static unsigned _SquishWriteTxt(HMSG hmsg, unsigned fAppend, byte * szTxt,
  dword dwTxtLen, dword * pdwOfs)
{
    ...
    /*
     *  If we're appending to existing text, make sure that we adjust
     *  properly.
     */
...
static unsigned _SquishUpdateIndex(HMSG hmsg, PXMSG pxm)
{
    ...
    /*
     *  Update this with the position of the message and the contents of
     *  the 'To:' field.
     */
...
sword apiSquishWriteMsg(HMSG hmsg, word fAppend, PXMSG pxm,
  byte * szTxt, dword dwTxtLen, dword dwTxtTotal, dword dwCtrlLen,
  byte * szCtrl)
{
    dword dwOfs = (dword) -1;
    ...
    /*
     *  If this was our first write to this message, update the index
     *  entry appropriately.
     */
