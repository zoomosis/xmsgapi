...
typedef struct
{
    ...
    SQIDX *psqi;  /* Pointer to index entries for this segment */
}
SQIDXSEG;
...
/*
 *  Private data in handle passed among API functions which handle
 *  message areas.
 */
...
struct _msgh
{
     ...
     *  wMode == MOPEN_CREATE:
     *
     *  If we are writing a completely new message, this field is
     *  zero.  Otherwise, if we are creating a message on top of an
     *  existing message, this will hold the frame offset of the old
     *  message.
     *
     *  wMode == MOPEN_RW or wMode == MOPEN_WRITE
     *
     *  This holds the offset of the message that we are rewriting
     *  (same as foWrite)
     */
     ...
     *
     *  If writing, this will hold the frame header of the message that
     *  we replaced.  (We only replaced a message if foRead != NULL_FRAME)
     */
     ...
     *  frame header.  If no frame header has been allocated for the 
     *  message yet, this field will be NULL_FRAME.
     */

    FOFS foWrite;

    /*
     *  If we are writing a message, this holds the frame that we wrote.
     *  This SQHDR is only valid when foWrite != NULL_FRAME.
     */
