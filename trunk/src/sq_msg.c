...
static HMSG NewHmsg(HAREA ha, word wMode)
{
    HMSG hmsg;
...
static unsigned _SquishHeaderValidRead(HAREA ha, SQHDR * psqh)
{
...
static dword _SquishTranslateNum(HAREA ha, dword dwMsg)
{
    if (dwMsg == MSGNUM_CUR)
    {
        return ha->cur_msg;
...
static unsigned _SquishSetCurFrameRead(HMSG hmsg, dword dwMsg, FOFS foCur, SQHDR * psqh)
{
...
static unsigned _SquishOpenMsgExisting(HMSG hmsg, dword dwMsg)
{
    SQHDR sqh;
    FOFS foMsg;
    ...
    {
        /*
         *  If the user tries to go to message zero, reset pointers
         *  appropriately
         */
    ...
    /*
     *  Read the frame header for this message and make sure that it's
     *  okay to read from this message.
     */
 ...
 *  message is in the process of being written.  To do this:
 *
...
static unsigned _SquishBlankOldMsg(HMSG hmsg, dword dwMsg)
{
    SQIDX sqi;
...
static unsigned _SquishBlankNewMsg(HMSG hmsg)
{
    SQIDX sqi;
...
static unsigned _SquishReduceMaxInternal(HAREA ha, dword * pdwDeleted,
  FOFS * pfoFirst, FOFS * pfoFirstPrior)
{
    SQIDX sqi;
    SQHDR sqh;
    ...
    while (ha->num_msg >= Sqd->dwMaxMsg)
    {
        ...
        *pfoFirst = sqh.next_frame;
        ...
        if (!_SquishRemoveIndexEntry(Sqd->hix, Sqd->wSkipMsg + 1L, NULL,
          &sqh, FALSE))
        {
            return FALSE;
        }
...
static unsigned _SquishReduceMaxPointers(HAREA ha, FOFS foFirst,
  dword dwDeleted, FOFS foFirstPrior)
{
    unsigned rc = TRUE;

    /*
     *  Adjust the 'first message' pointer.  If we have no skip_msgs, just
     *  set the pointer for the beginning of the area.  If we do have
    ...
    if (ha->cur_msg > (dword) Sqd->wSkipMsg)
    {
        if (ha->cur_msg <= (dword) Sqd->wSkipMsg + dwDeleted)
        {
            ...
            /*
             *  We were inside the range of messages that was deleted.  To
             *  handle this, simply set our pointer to the first message
        ...
        {
            /*
             *  We were above the deleted range, so we will have to
             *  decrement our message number.
             */
    ...
    if (ha->high_water >= (dword) Sqd->wSkipMsg)
    {
...
/*
 *  Delete enough messages in this area so that we fall below the dwMaxMsg
 *  limit.
 *
 *  This function assumes that we have exclusive access to the Squish base.
 */

static unsigned _SquishReduceMaxMsgs(HAREA ha)
{
    ...
    if (!Sqd->dwMaxMsg || ha->num_msg < Sqd->dwMaxMsg ||
...
static unsigned _SquishOpenMsgCreate(HMSG hmsg, dword dwMsg)
{
    unsigned rc = TRUE;

    /*
     *  If we are creating a completely-new message, we need to adjust the
     *  header and index file IMMEDIATELY to show that we want this new
     *  message number.
     */
...
HMSG apiSquishOpenMsg(HAREA ha, word wMode, dword dwMsg)
{
    HMSG hmsg;
    unsigned fOpened;
...
/*
 *  This function undoes what SquishOpenMsg did when creating a new message.
 *  This is only called if the write was not completed.
 */

static unsigned _SquishCloseUndoWrite(HMSG hmsg)
{
    if (!_SquishExclusiveBegin(hmsg->ha))
    {
        return FALSE;
...
static unsigned _SquishCloseRemoveList(HMSG hmsg)
{
...
sword apiSquishCloseMsg(HMSG hmsg)
{
